...
const Dashboard = () => {
  ...
  const uncompleted = useTasksStore(state => state.getUncompletedCount())
  ...
  return (
    <section className="p-4 mb-4 bg-gray-100 rounded shadow-md">
      ...
      <p>🕒 Remaining: {uncompleted}</p>

      {total === 0 && <p className="italic mt-2">Start by adding your first task!</p>}
      {total > 0 && uncompleted === 0 && <p className="mt-2 text-green-600 font-medium">You're all caught up! 🎉</p>}
    </section>
  )
}
